...
/* Indigomine customizations JS
   ...
   近傍の .issue.tracker-N からトラッカー番号を読んでチップにクラスを付与する。
   （h2 は .issue.tracker-N の外＆1つのテキストなので、色分けと分割には JS が要る。） */
(function () {
  function decorate() {
    var h2 = document.querySelector('#content h2.inline-block');
    if (!h2 || h2.dataset.imChip) return;
    var m = h2.textContent.trim().match(/^(.+?)\s+#(\d+)\s*$/);
    if (!m) return;

    var tk = '';
    var issueEl = document.querySelector('#content .issue[class*="tracker-"]');
    if (issueEl) {
      var cm = issueEl.className.match(/tracker-(\d+)/);
      if (cm) tk = cm[1];
    }

    h2.dataset.imChip = '1';
    h2.classList.add('im-idchip');
    if (tk) h2.classList.add('im-tk-' + tk);

    var t = document.createElement('span'); t.className = 't'; t.textContent = m[1];
    var n = document.createElement('span'); n.className = 'n'; n.textContent = '#' + m[2];
    h2.textContent = '';
    h2.appendChild(t);
    h2.appendChild(n);
  }
